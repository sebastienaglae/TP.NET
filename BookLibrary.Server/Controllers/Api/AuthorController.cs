...
public class AuthorController(LibraryDbContext libraryDbContext, DtoMapper mapper) : ApiController
{
    ...
    public async Task<GetAuthorsResponse> GetAuthors([FromQuery] GetAuthorsRequest request)
    {
        ...
        if (!string.IsNullOrWhiteSpace(request
                .Query)) // concatenate first and last name and search for the query (case-insensitive)
            authorQuery =
                authorQuery.Where(a => EF.Functions.Like($"{a.FirstName} {a.LastName}", $"%{request.Query}%"));
        if (request.Offset > 0)
    ...
    {
        var author = await libraryDbContext.Authors.FirstOrDefaultAsync(b => b.Id == id);
        if (author is null)
            return NotFound();
